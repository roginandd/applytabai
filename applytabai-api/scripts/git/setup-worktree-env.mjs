...
const projectHookPath = resolve(projectDir, '.githooks', 'post-checkout');
const projectUpdaterPath = resolve(projectDir, 'scripts', 'git', 'update-worktree-env.mjs');

if (!existsSync(hookPath) || !existsSync(updaterPath)) {
  if (existsSync(projectHookPath) && existsSync(projectUpdaterPath)) {
    run('git', ['config', 'core.hooksPath', relative(repoRoot, resolve(projectDir, '.githooks'))], { cwd: repoRoot, stdio: 'ignore' });
    execFileSync('node', ['scripts/git/update-worktree-env.mjs'], { cwd: projectDir, stdio: 'inherit' });
